...
namespace gl {
    ...
    public:
        TreeWithDataTestFixture() {
            ...
            test_leaf = make_shared<TestTreeWithData<dp::Test_Data, dp::Empty_Config>>(x, y, width, height, config, 0);
        }
    ...
    BOOST_FIXTURE_TEST_CASE(TestConstructorOfDataTree, TreeWithDataTestFixture) {
        BOOST_TEST(test_leaf->getMaxDepth() == 0);
    ...
    BOOST_FIXTURE_TEST_CASE(TestChildConstructorOfDataTree, TreeWithDataTestFixture) {
        ...
        BOOST_TEST(test_child->getMaxDepth() == 0);
