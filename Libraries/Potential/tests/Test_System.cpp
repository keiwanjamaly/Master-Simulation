...
namespace phy {
    ...
    public:
        ...
        static void observer(grid &points, double t) {
//            std::cout << t << "\t" << points[int(points.size() / 2)] << std::endl;
        }
    ...
    TEST_F(HeatEquationTestFixture, testConstructor) {
        heat_solver = System(x_points, diffusion, left_boundary_condition,
    ...
    TEST_F(HeatEquationTestFixture, RHStest) {
        ...
        for (int i = 1; i < N_tmp - 1; i++) {
            EXPECT_FLOAT_EQ(result[i],
                            ...
                            (dx * dx)) << "Vectors x and y differ at index " << i;
        }
    ...
    TEST_F(HeatEquationTestFixture, heatEquation) {
        heat_solver = System(x_points, diffusion, left_boundary_condition,
                             right_boundary_condition);
        integrate_adaptive(make_controlled<error_stepper_type>(1.0e-11, 1.0e-7),
                           heat_solver, temperature, t_min, t_max, 0.01, observer);
        for (int i = 0; i < N; i++) {
